-- Marketing Sloganizer Database Schema (Clean Install)
...
DROP POLICY IF EXISTS "Users can view their own saved slogans" ON saved_slogans;
DROP POLICY IF EXISTS "Users can insert their own saved slogans" ON saved_slogans;
DROP POLICY IF EXISTS "Users can update their own saved slogans" ON saved_slogans;
DROP POLICY IF EXISTS "Users can delete their own saved slogans" ON saved_slogans;
DROP POLICY IF EXISTS "Users can view their own generation history" ON generation_history;
DROP POLICY IF EXISTS "Anyone can insert generation history" ON generation_history;
...
ON CONFLICT (name) DO UPDATE SET
    monthly_slogan_limit = EXCLUDED.monthly_slogan_limit,
...
DROP TRIGGER IF EXISTS update_saved_slogans_updated_at ON saved_slogans;
